...
def build_argparser():
    ...
    args.add_argument("-i", "--input", default=0, help="Required. Path to an image/video file. (Specify 'cam' to work with "
                                            "camera)", type=str)
    ...
    args.add_argument('--output', action='store', type=str, default='output.mp4')
...
#            sys.exit(1)
    ...
    while cap.isOpened():
        ...
        for obj in objects:
            ...
            label = (' '+names[int(det_label)] + ' ' + str(round(obj['confidence'] * 100, 1)) + ' %')
            text_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 2, 1)
            dim, baseline = text_size[0], text_size[1]
            cv2.rectangle(frame, (obj['xmin'], obj['ymin']), (obj['xmin'] + dim[0] //3, obj['ymin'] - dim[1] + baseline), color3, cv2.FILLED)
